 ...
 * 
 */
const GET_USERINFOS = "getUserInfos";
...
function GetUserInfosBDD(){
	...
	maRep.send('action=' + GET_USERINFOS +
				'&user_mail=' + USERMAIL);
}
...
function extraireInfosUser(infos) {
	...
	var AthleteNomPrenom = infos["athl_prenom"];
	var AthleteNomID = infos["athl_id"];
...
function Add_Activity(athleteEmail) {
	
	var x = document.getElementById("custMail");
	x.setAttribute("value", athleteEmail);
...
function Add_Structure_TableOneWeek(year, Nbweeks){
	...
	for (i = 1; i <= Nbweeks; i++){
		...
		var span = document.createElement("span");
		s.appendChild(span);
		var title = document.createTextNode('0 m');
		span.appendChild(title);
...
function allowDrop(ev) {
	...
	function sendPasteActivity(custMail, activityTitle, activityType, activitySport, activityDate, activityDuration, activityDistance, activityElevation, activityDescription){
		var url = URL_CONTROL;
