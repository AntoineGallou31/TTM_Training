...
let USERMAIL = document.getElementById("user_mail").value;
let athleteMail = document.getElementById("custMail").value;
...
(function(){
	...
	document.getElementById("add_activity_form2").addEventListener("submit", ModifyActivity, false);
...
function sendAjaxActivity(USERMAIL, activityTitle, activityType, activitySport, activityDate, activityDuration, activityDistance, activityElevation, activityDescription){
	var url = URL_CONTROL;
...
function ShowAllActivities(athleteMail){
	GetAllActivities(athleteMail);
}
...
function get_all_activitiesJSON(reply) {
	...
	function getShowingActivity(id){
		...
		sendAjaxShowingActivity(id, USERMAIL);
	}
